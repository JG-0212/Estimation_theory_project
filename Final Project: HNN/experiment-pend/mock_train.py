...
def train(args):
    ...
    for step in tqdm(range(args.total_steps+1)):
        ...
        if args.new_loss:
            t_eval = torch.linspace(0, 3, steps=n_points)
            batch_loss = 0.0
            n_samples = len(data['x'])  # should be T

            for i in range(0, n_samples, batch_size):
                traj = data['x'][i : i + batch_size]  # shape (T,2)
                y0 = traj[0]                          # shape (2,)
                hnn_x_hat = integrate_model_rk4(model, y0, t_eval)  # [T,2]
                hnn_x     = torch.tensor(traj, dtype=torch.float32)
                batch_loss += L2_loss(hnn_x, hnn_x_hat)

            loss = loss + batch_loss / n_samples

            if args.verbose and step % args.print_every == 0:
                print(f"Loss of term 2 {batch_loss}")
...
def integrate_model_rk4(model, y0, t_eval):
    # y0: array or tensor of shape (2,)
    y = torch.tensor(y0, dtype=torch.float32).view(1, -1)  # now [1,2]
    y = y.requires_grad_(True)

    trajectory = [y]         # list of [1×2]
    dt = t_eval[1] - t_eval[0]

    for _ in range(1, len(t_eval)):
        dy = rk4(model.time_derivative, y, torch.tensor(0.0), dt)
        y = (y + dy).requires_grad_(True)
        trajectory.append(y)

    # stack -> [T,1,2] -> reshape to [T,2]
    traj = torch.stack(trajectory, dim=0)
    return traj.view(len(t_eval), -1)
