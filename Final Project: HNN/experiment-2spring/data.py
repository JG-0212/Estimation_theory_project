...
def hamiltonian_fn(coords):
    ...
    H = 2 + (q**2-2*((1+q**2)**.5)) + 0.5*(p**2) # H = 2*0.5*k((l**2+q**2)**.5-l)**.5 + p**2/(2*m) l =1, m = 1,k
    return H
...
def get_trajectory(t_span=[0,3], timescale=35, radius=None, y0=None, noise_std=0.1, **kwargs):
    ...
    if radius is None:
        radius = np.random.rand() + 1.0 # sample a range of radii
